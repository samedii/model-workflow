...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return dist.Bernoulli(logits=x)
...
def train(config):
    ...
    def loss_func(features, labels):
        # optional: add model diagnostics

        dist = model(features)
        return mnist_problem.metrics.get_loss(features, labels, dist)

    # didactic_meme.fit(mnist_problem, model, loss_func, config)
